...
public class Ladder {
    public static void main(String[] args) {
        ...
        List<List<String>> res = ladder.findLadders("cet", "ism", new ArrayList<String>(Arrays.asList(new String[] {
    ...
    private List<String> findCharChg(String beginWord, Set<String> wordList) {
        List<String> res = new ArrayList<>();
        for (String str : wordList) {
            if (onlyOneChange(str, beginWord)) {
                res.add(str);
            }
        }
        return res;
    ...
    private boolean onlyOneChange(String str, String beginWord) {
        if (str.length() != beginWord.length()) {
            return false;
        }
        int change = 0;
        for (int i = 0; i < str.length() && change <= 1; i++) {
